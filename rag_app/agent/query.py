...
- **Do NOT use internal-protocol entry IDs like `[B-CC-021]`, `[B-DT-022]`, `[B-TF-014]`, `[DX-001]`, `[RULE-005]`, `[risk-id-XXX]` etc. as if they were citations.** These are internal markers that may appear in the evidence block but they are NOT PubMed references — they index into the protocol skill file. Citations are ONLY plain integers like `[1]`, `[2]`, `[2, 3]` that index into the "Ranked PubMed evidence" block. Any bracketed token containing letters or hyphens (e.g. `[B-CC-021]`, `[DX-004]`, `[RULE DX-009]`) is an internal artifact — never put it in your output, write the parameter or guidance as plain prose instead.
