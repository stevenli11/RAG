...
- **Two DIFFERENT citation systems — use both correctly:**
  - **PubMed citations**: ONLY plain integers like `[1]`, `[2]`, `[2, 3]` indexing into the "Ranked PubMed evidence" block. Use for facts supported by abstracts. Mandatory wherever an abstract genuinely supports the claim.
  - **Internal rule references**: When the evidence block or the objective-audit block mentions a specific failure-mode rule (e.g. `[B-CC-021]`, `[B-DT-022]`, `[DX-001]`, `[DX-004]`, `[RULE DX-003]`), you MAY inline that rule's ID in brackets EXACTLY as written in the evidence — the UI will render hover tooltips showing the rule description. Only use rule IDs that appear verbatim in the evidence/audit blocks; never invent new ones (e.g. `[B-XX-999]` you made up — the UI will strip these as hallucinations).
- These two systems never collide: PubMed cites are always pure digits inside brackets, rule refs always contain letters and hyphens.
